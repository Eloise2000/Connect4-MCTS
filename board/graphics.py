    ...
    def draw_gboard(self, board):
        for c in range(board.COLUMN_COUNT):
            for r in range(board.ROW_COUNT):
                pygame.draw.rect(self.screen, self.GREY, (c*self.SQUARESIZE, r*self.SQUARESIZE+self.SQUARESIZE, \
        ...
        for c in range(board.COLUMN_COUNT):
            for r in range(board.ROW_COUNT):		
                if board.get_row_col(r, c) == 1:
                    pygame.draw.circle(self.screen, self.BLUE, (int(c*self.SQUARESIZE+self.SQUARESIZE/2),\
                         self.height-int(r*self.SQUARESIZE+self.SQUARESIZE/2)), self.RADIUS)
                elif board.get_row_col(r, c) == 2: 
                    pygame.draw.circle(self.screen, self.GREEN, (int(c*self.SQUARESIZE+self.SQUARESIZE/2), \
                        self.height-int(r*self.SQUARESIZE+self.SQUARESIZE/2)), self.RADIUS)
        self.update_gboard()
